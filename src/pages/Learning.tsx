...
const speak = (text?: string, forceUnlock = false) => {
  // 🔐 SAFETY LAYER 1: Trim and validate input
  ...
  const now = Date.now();
  if (safe === lastSpoken && now - lastSpokenAt < 800) return;

  lastSpoken = safe;
  lastSpokenAt = now;
  
  // 🔐 SAFETY LAYER 2: Always stop before speaking (prevents overlaps and freezes)
  try {
